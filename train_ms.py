...
def run(rank, n_gpus, hps):
  ...
  if getattr(hps.model, "use_duration_discriminator", False):
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(train_loader):
    ...
    with autocast('cuda', enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q), (hidden_x, logw, logw_) = net_g(x, x_lengths, spec, spec_lengths, speakers, bert=bert)
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        if net_g.module.use_noise_scaled_mas:
          scalar_dict.update({"mas_noise_scale": net_g.module.current_mas_noise_scale})
...
def evaluate(hps, generator, eval_loader, writer_eval, plbert=None):
    ...
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(eval_loader):
        ...
        break
      bert = plbert(x, x_lengths) if plbert is not None else None
      y_hat, attn, mask, *_ = generator.module.infer(x, x_lengths, speakers, max_len=1000, bert=bert)
